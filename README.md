<img width="822" height="419" alt="image" src="https://github.com/user-attachments/assets/ecc24add-2412-43b0-b241-8175a7ed5a63" />

## Bakery Shop Website

## Overview

The Bakery Shop Website is a modern and responsive website designed to showcase a bakery's products, services, and contact information. The website includes the menu of items in the shop, contact info, stuning UI.

## Features

* Responsive design for desktop, tablet, and mobile devices.
* Home page with bakery products.
* Products menu displaying different types of breads with its price.
* About Us section introducing the ethics of the shop.
* Contact section with adress, phone number, email, and social media links.

## Technologies Used

* HTML5
* CSS3
* JavaScript

## Usage

Developed as a bakery shop website project by enhancing the marketing startegy of the shop.

## Author

Abobakr Essam


## Website Pictures
<img width="437" height="366" alt="image" src="https://github.com/user-attachments/assets/1088edd8-b0ac-4840-ba72-cb4c9b43bc65" />
<img width="454" height="253" alt="image" src="https://github.com/user-attachments/assets/0e38d24a-0005-4cda-8ef3-2512fefaed5d" />
<img width="699" height="421" alt="image" src="https://github.com/user-attachments/assets/20c43e7e-948d-4f60-b60d-5fbaf48d72da" />
<img width="803" height="368" alt="image" src="https://github.com/user-attachments/assets/3531bde4-0e9d-4e3d-b8ef-09ba9d12b6e0" />



